...
import { getLoadDraft, getPost, getSaveDraft } from "../../../modules/post/service/postService";
import toast from "react-hot-toast";
import html2md from "html-to-md";
import SchedulePost from "../write/schedule";
import MDXEditor from "../write/MDX/editor";
import AddTag from "../write/tag";
import { useParams } from "react-router-dom";
import PublishButton from "./publish";
import PageLoading from "../../loading/loading";
import urlSlug from 'url-slug'
import showError from "../../error/show";
...
const EditPost = () => {
    ...
    useEffect(() => {
        ...
        getPost(params.slug ? params.slug : '').then((res) => {
            setId(res.data.id)
    ...
    useEffect(() => {
        if (done === true) {
            window.location.href = '/' + urlSlug(title)
        }
    }, [done]);
    if (!isLoggedIn) {
